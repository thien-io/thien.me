...
export function SpotifyWidget() {
  ...
  if (!data) {
    return (
      <div className="h-16 border border-border bg-muted animate-pulse" />
    );
  }
  ...
  return (
    <a
      ...
      className="flex items-center gap-3 p-4 border border-border bg-card transition-colors hover:border-primary/50 group"
    >
      {data.albumArt && (
        <Image
          src={data.albumArt}
        ...
        <div className="flex items-center gap-1.5 mb-0.5">
          <div className="flex items-end gap-[2px] h-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="w-[3px] bg-primary spotify-bar"
